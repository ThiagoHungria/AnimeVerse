...
export function AnimeCard({ anime, className, priority }: AnimeCardProps) {
  ...
  return (
    ...
    >
      <Link
        href={`/anime/${anime.id}`}
        className="group relative block h-full overflow-hidden rounded-xl border border-border bg-card shadow-none transition-shadow duration-300 hover:border-border-strong hover:shadow-xl hover:shadow-black/40"
      >
